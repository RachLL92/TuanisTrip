/* =====================================================
reserva.js
Lógica de reserva.html (Página de Registro/Gestión):

1. Carrito: lee data/tours.json + el carrito guardado por
   carrito.js, permite sumar/restar cantidad y eliminar
   ítems, y calcula el total.
2. Formulario: valida los datos del cliente en tiempo
   real y al enviar (sin usar alert ni recargar).
3. Al confirmar: guarda la reserva completa en
   localStorage, vacía el carrito y avisa al usuario.
4. Mis reservas: lista lo guardado, permite marcarlo como
   confirmado, eliminar una reserva o cancelarlas todas
   (con confirmación visual, no con confirm() del navegador).
===================================================== */
...
/* Recalcula el total a partir de lo que hay en localStorage
en este momento (no de lo que está pintado en pantalla). */
...
/* El botón "Confirmar reserva" no tiene sentido si el
carrito está vacío, así que se deshabilita solo. */
function actualizarEstadoBotonConfirmar() {
    btnConfirmarReserva.disabled = obtenerCarrito().length === 0;
}
...
/* Delegación de eventos: +, − y eliminar, todo dentro del
mismo contenedor (las tarjetas se vuelven a pintar cada
vez, así que no sirve poner un listener por botón). */
contenedorCarritoReserva.addEventListener("click", (evento) => {
    ...
    actualizarContadorCarrito(); // viene de carrito.js (burbuja del header)
    pintarCarrito();
});
...
function validarFechaReserva() {
    if (!inputFechaReserva.value) {
        mostrarError(inputFechaReserva, "Selecciona la fecha en la que quieres viajar.");
        return false;
    }

    mostrarError(inputFechaReserva, "");
    return true;
}
...
function validarFormularioReserva() {
    ...
    const fechaValida = validarFechaReserva();
    return nombreValido && correoValido && fechaValida;
}

/* Validación en tiempo real, mientras el usuario escribe. */
inputNombre.addEventListener("input", validarNombre);
inputCorreo.addEventListener("input", validarCorreo);
inputFechaReserva.addEventListener("change", validarFechaReserva);
...
vacía el carrito (ya quedó "convertido" en la reserva).
===================================================== */
...
formReserva.addEventListener("submit", (evento) => {
    ...
    formReserva.reset();
    [inputNombre, inputCorreo, inputFechaReserva].forEach(input => mostrarError(input, ""));
        ...
        <article class="reserva-card" data-id-reserva="${reserva.id}">
            ...
            <ul class="reserva-items">
                ${reserva.items.map(item => `
                    <li>${item.cantidad} × ${item.nombre} — ${formatoColones(item.subtotal)}</li>
                `).join("")}
            </ul>
...
/* Delegación de eventos para "Marcar confirmada" y eliminar
...
/* "Cancelar todas": confirmación visual en pantalla, no
con confirm() del navegador (igual criterio que el resto
del sitio: nada de alert/confirm nativos). */
...
impedirFechasPasadas(inputFechaReserva); // viene de utilidades.js
cargarTours();